...
namespace SimpleScene.Util3d {
    ...
    public class VertexSoup<VERTEX_STRUCT> {
        ...
        Dictionary<VERTEX_STRUCT,UInt32> vertexToIndexMap = new Dictionary<VERTEX_STRUCT,UInt32>();
        public List<VERTEX_STRUCT> verticies = new List<VERTEX_STRUCT>();
        private readonly bool deDup;

		public UInt32 digestVertex(ref VERTEX_STRUCT vertex) {
			UInt32 retval;
		    if (deDup && vertexToIndexMap.ContainsKey(vertex)) {
                retval = vertexToIndexMap[vertex];
            } else {
                UInt32 nextIndex = (UInt32)verticies.Count;
        ...
        public UInt32[] digestVerticies(VERTEX_STRUCT[] vertex_list) {
            UInt32[] retval = new UInt32[vertex_list.Length];
